...
public class APIStatistics {
	...
	public APIMessage listDistributions(int skip, int limit, int searchVocabularies, String search,
		...
		if (((searchSubject != null) || (searchProperty != null) || (searchObject != null)))
			hasResource = true;
		...
		for (DistributionDB d : distributions) {
			...
			// jsonObj.put(new
			// DatasetMongoDBObject(d.getDefaultDatasets().get(0)).getUri());
	...
	public APIMessage getDatasetDetailsStatistics(int distribution, int topN, String type) {
		APIMessage apimessage = new APIMessage();
	...
	public APIMessage datasetStatistics(int distribution, int topN, String type) {
		APIMessage apimessage = new APIMessage();
		NumberFormat formatterLinks = new DecimalFormat("###,###,###,###");
	...
	public APIMessage getTopNLinks(int dataset1URL, int dataset2URL, String type) {
		...
		LinkedHashMap<String, Integer> links = new TopNLinksQueries().getTopNLinks(dataset1URL, dataset2URL,
				collectionName);
	...
	public APIMessage compareDatasets(int dataset1URL, int dataset2URL, String type) {
		...
		// String dataset1URL =
		// "https://raw.githubusercontent.com/AKSW/n3-collection/master/Reuters-128.nt";
		// String dataset2URL =
		...
		for (String d : m.keySet()) {
			// DistributionDB datasetDB = new
			// DistributionDB(d.getDistributionTarget());
			JSONArray jsonObj = new JSONArray();
			// jsonObj.put(d.getDistributionTarget());
			jsonObj.put(d);
			jsonObj.put(m.get(d));
